...
extern void gpio(char port,char pinSelect,char direction,char pull_up,char gpiois,char gpioibe,char gpioiev,char gpioim); 
extern char keyGet(char* readPort,char bitsInRead);
extern char keyMatrix(char* scanPort,char bitsInScan,char* readPort,char bitsInRead);
extern void startTimer(int limit);
extern void stopTimer();
extern void editTimer();
extern void lcd_begin_init(void);
extern void screen_begin(void);
extern void screen_clear(void);
extern void lcd_update_screen(int low, int high, int pot, int sen);

void init_begin(){
	SYSCTL->RCGCGPIO |= 0x30;
	SYSCTL->RCGCADC |=0x01;
	
	 GPIOE->AFSEL |= (0x0C);// eskisi GPIOE->AFSEL |= (1<<3); //yeni
	 GPIOE->DIR &= ~0x0C; // eskisi GPIOE->DIR |= 0x08; //yeni
	 GPIOE->DEN &= ~(0x0C); // eskisi GPIOE->DEN &= ~(0x08); //yeni
	 GPIOE->AMSEL |= (0x0C); // eskisi GPIOE->AMSEL |= (1<<3); //yeni
...
void update_rgb_led(int data, int lower_limit, int higher_limit){
...
void update_pumps(int data, int lower_limit, int higher_limit){
...
int main(){
	...
	//OutStr(arr);
	gpio('F',0x0E,0x0E,0x00,0x00,0x00,0xFF,0x00);//
	gpio('B',0x0C,0x0C,0x00,0x00,0x00,0xFF,0x00);//B[2..3] pump switch output, B[4..7] botton input
	gpio('E',0x33,0x00,0x00,0x00,0x00,0xFF,0x00);//port E pins 0,1,4,5 are used for keypad outputs
	gpio('D',0x0F,0x0F,0x00,0x00,0x00,0xFF,0x00);//port D pins 0-3 are used for keypad inputa
	...
	while(1){
		...
		switch(pot_set){
			...
			case 3:if(0 < key_matrix && key_matrix < 10){pot_set = 5;lower_l = key_matrix*10;}break;
			case 4:if(0 < key_matrix && key_matrix < 10){pot_set = 6;upper_l = key_matrix*10;}break;
			
			case 5:if(key_matrix==0)pot_set = 7;break;
			case 6:if(key_matrix==0)pot_set = 8;break;

			case 7:if(0 < key_matrix && key_matrix < 10){pot_set = 0;lower_l += key_matrix;}break;
			case 8:if(0 < key_matrix && key_matrix < 10){pot_set = 0;upper_l += key_matrix;}break;
			default:break;
		}
		update_rgb_led((sensor*99)/4095,lower_l,upper_l);
		update_pumps((sensor*99)/4095,lower_l,upper_l);
		sprintf(arr,"key: %d pot_s: %d\r\4\n",key_matrix,pot_set);OutStr(arr); //debug
		sprintf(arr,"S:%d P:%d LL:%d UL: %d\r\4\n",(sensor*99)/4095,(pot*99)/4095,lower_l,upper_l);
		OutStr(arr);
		lcd_update_screen(lower_l,upper_l,(pot*99)/4095,(sensor*99)/4095 );
		//LCD_WRITE(sensor/SENSOR_SCALE,lower_l,upper_l);	//LCD is refreshed
		}
